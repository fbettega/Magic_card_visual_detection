...
X_hog, y_layout,label_encoder = CardImageProcessor(images_dir = images_dir).load_images_parallel(filtered_cards, target_y="border_color", max_per_y=5000)
#################################################################################################################################################################################
...
rarity_counts = Counter(getattr(card, "border_color") for card in filtered_cards.values())
# Afficher les résultats
for rarity, count in rarity_counts.items():
    print(f"{rarity}: {count}")
