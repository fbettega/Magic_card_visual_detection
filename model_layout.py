...
def load_images_parallel(filtered_cards):
    ...
    y_rarity = le_rarity.fit(y_rarity).transform(y_rarity)
...
for train_idx, test_idx in sss.split(X_hog, y_rarity):  # Stratification sur y_rarity
...
def test_y_rep(cards,y_train,y_test,var_name):
    rarity_counts = Counter(card.get("rarity") for card in cards.values())
...
X_train_hog_gpu = X_train_hog
# del X_train_hog
X_test_hog_gpu = X_test_hog
